<script lang="ts">
  ...
  onMount(async () => {
    ...
    let chart = new chartjs(chartCanvas, {
      data: {
        labels: hydroChartDataFetch.map((val) =>
          moment(val.x).format("MMM DD")
        ),
        datasets: [
          {
            type: "line",
      ...
      options: {
        ...
        scales: {
          ...
          C: {
            ...
            ticks: {
              color: "#ffffff80",
              font: { family: "Roboto Mono", size: 8 },
            },
            min: 50000,
            max: 150000,
          },
  ...
  const hydroChart = async () => {
    ...
    if (!res.ok) {
      ...
      const fetchHydroVolume = priceData.total_volumes.map(
        (value: number[]) => ({
          x: value[0],
          y: value[1].toFixed(1),
        })
      ...
      const fetchHydroMarketcap = priceData.market_caps.map(
        (value: number[]) => ({
          x: value[0],
          y: value[1].toFixed(1),
        })
